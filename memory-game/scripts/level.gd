...
func _new_game() -> void:
	$background.texture = load(str("res://assets/backgrounds/BG_",( (randi() % 6) + 1 ),".png")) 
	# reset das variaveis
	face_cards = 0
	card_opened.clear()
	combo = 0
	...
	var deck = []
	for i in range(qtd_cards / 2): # dividido por 2 porque as outras duas vão ser duplicadas
		# duplico as cartas
		deck.append(cards[i])
		deck.append(cards[i])
